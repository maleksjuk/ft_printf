...
/*   Updated: 2019/12/01 20:59:22 by obanshee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_infinity(t_printf **p, double num)
{
	if (num == 1 / -0.0)
	{
		(*p)->final_str = ((*p)->uppercase == 1) ?
			ft_strjoin_len((*p)->final_str, "-Inf", 4, (*p)->final_len) :
			ft_strjoin_len((*p)->final_str, "-inf", 4, (*p)->final_len);
		(*p)->final_len += 4;
	}
	else if (num == 1 / 0.0)
	{
		(*p)->final_str = ((*p)->uppercase == 1) ?
			ft_strjoin_len((*p)->final_str, "Inf", 3, (*p)->final_len) :
			ft_strjoin_len((*p)->final_str, "inf", 3, (*p)->final_len);
		(*p)->final_len += 3;
	}
	else if (num != num)
	{
		(*p)->final_str = ((*p)->uppercase == 1) ?
			ft_strjoin_len((*p)->final_str, "Nan", 3, (*p)->final_len) :
			ft_strjoin_len((*p)->final_str, "nan", 3, (*p)->final_len);
		(*p)->final_len += 3;
	}
	else
...
int		ft_f(t_printf **p)
{
	...
	if (!(str = ft_strnew(tab[1] + 1)))
		return (1);
	if ((*p)->minus)
		tab[2] = f_minus(p, &num, str, tab);
	else if ((*p)->zero)
		tab[2] = f_zero(p, &num, str, tab);
	else
		tab[2] = f_def(p, &num, str, tab);
